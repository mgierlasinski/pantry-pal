...
public class RecipeService : IRecipeService
{
    ...
    {
        var response = await _httpClient.GetAsync("recipe-reject-reasons");
    ...
    {
        var response = await _httpClient.PostAsync("recipes/generate", null);
        response.EnsureSuccessStatusCode();
    ...
    {
        var response = await _httpClient.PostAsync($"recipes/{generationId}/accept", null);
        response.EnsureSuccessStatusCode();
    ...
    public async Task RejectRecipeAsync(string generationId, RecipeRejectRequestDto payload)
    {
        ...
        var response = await _httpClient.PostAsync($"recipes/{generationId}/reject", content);
        response.EnsureSuccessStatusCode();
    }

    public async Task<RecipesPaginatedResponseDto> GetRecipesAsync(int page, int pageSize)
    {
        var response = await _httpClient.GetAsync($"recipes?page={page}&pageSize={pageSize}");
        response.EnsureSuccessStatusCode();
